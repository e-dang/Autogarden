...
protected:
    ...
    OutputPinSetTest() : pinsOrig(size) {
        ...
        pinSet = std::make_unique<OutputPinSet<NiceMock<MockTerminalPin>*>>(std::move(pins));
    }
};
...
TEST_F(OutputPinSetTest, getNextAvailable) {
    ...
    for (size_t i = 0; i < pinsOrig.size(); i++) {
        auto& pin = pinsOrig[i];
...
TEST_F(OutputPinSetTest, getNumAvailable) {
    ...
    auto count = getNumAvailableSetUp(pinsOrig, mode);
...
TEST_F(OutputPinSetTest, hasNumAvailableReturnsFalse) {
    auto mode         = PinMode::Digital;
    auto numAvailable = getNumAvailableSetUp(pinsOrig, mode);
...
TEST_F(OutputPinSetTest, getPinNumbers) {
    std::for_each(pinsOrig.begin(), pinsOrig.end(), [](auto& pin) { EXPECT_CALL(pin, getPin()); });

...
public:
    ...
    std::unique_ptr<ITerminalPinSet> buildTerminalPinSet(const std::initializer_list<int>& digitalOutputPins,
        ...
        return std::make_unique<TerminalPinSet>(pins);
    }

    void addPins(const std::initializer_list<int>& pinNums, const PinMode& pinMode, std::vector<ITerminalPin*>* pins) {
        for (const auto& pinNum : pinNums) {
            pins->push_back(new TerminalPin(pinNum, pinMode));
        }
    }
};
